...
export const healFull = (characters) =>
...
const bufferAnimation = (combatant, vfx, timestamp) => {
...
const bufferAudio = (audio, sfx, timestamp) => {
...
const injectAnimation = (combatant, target, currentAbility, now, teams, events = []) => {
...
const pickAbility = (abilities, ability, now, startOrEnd) =>
...
const prioritySorting = (combatants, now, startOrEnd) =>
  combatants.sort((a, b) => sortByAbilityPriority(a, b, now, startOrEnd));

const tickStatusEffects = (combatants, now) => {
...
export const generateCombat = (teams, seed = `${Math.random()}`, fightId) => {
