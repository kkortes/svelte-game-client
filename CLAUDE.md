...
- **Direct attribute-name binding over redundant `attr-name="@[obj.attr-name]"`**: when the attribute name and the binding expression's leaf are the same name (e.g. `name="@[ability.name]"`), use Vibe's name-binding form `<element @[ability.name]>` directly — it sets the attribute named after the resolved value. This is the same pattern as `<icon @[ability.icon]>`. The redundant value-binding form would be treated as a boolean attribute by Vibe (set to empty when truthy) so it doesn't even carry the value through, and the duplication is noise. For passing data the markup can't shape into an attribute name (numbers, free text, etc.), prefer inline interpolation in `on*` handlers — e.g. `onmouseenter="showFoo(this, '@[obj.name]', @[obj.ticks])"` — since `on*` attribute values are string-templated, not boolean-coerced.
